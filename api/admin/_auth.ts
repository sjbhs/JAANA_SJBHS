...
const adminPasswordValue = process.env.ADMIN_PASSWORD?.trim() || "";
...
function createCookieValue(name: string, value: string, maxAgeSeconds: number) {
  return `${name}=${value}; Path=/; HttpOnly; SameSite=Strict; Secure; Max-Age=${maxAgeSeconds}; Priority=High`;
}
...
export function getAdminSessionFromCookie(cookieHeader: string | null | undefined) {
  ...
  const token = readCookies(cookieHeader)[sessionCookieName];
