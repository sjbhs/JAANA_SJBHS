...
alter table merch_reservation_items
  add column if not exists unit_price_usd numeric(10, 2) not null default 0;

insert into merch_products (sku, name, total_quantity, price_usd, product_type, sort_order)
values
  ('MERCH-001', 'Circular Josephite Magnet', 40, 10, 'individual', 1),
  ('MERCH-002', 'Sticker Set', 50, 10, 'individual', 2),
  ('MERCH-003', 'Luggage Tag', 50, 10, 'individual', 3),
  ...
  ('MERCH-023', 'Bamboo Water Sipper', 35, 40, 'individual', 23),
  ('MERCH-024', 'Scarf', 30, 20, 'individual', 24),
  ('MERCH-025', 'Cap', 30, 20, 'individual', 25),
  ('BUNDLE-001', 'Josephite Starter Bundle', 40, 35, 'bundle', 101),
  ('BUNDLE-002', 'Alumni Essentials Bundle', 25, 60, 'bundle', 102),
  ('BUNDLE-003', 'Work & Travel Bundle', 15, 60, 'bundle', 103),
  ('BUNDLE-004', 'Heritage Bundle', 20, 90, 'bundle', 104),
  ('BUNDLE-005', 'Art & Memory Bundle', 20, 70, 'bundle', 105),
  ('BUNDLE-006', 'Premium Josephite Gift Bundle', 20, 180, 'bundle', 106),
  ('BUNDLE-007', 'Andrews House Pride Bundle', 20, 35, 'bundle', 107),
  ('BUNDLE-008', 'Davids House Pride Bundle', 20, 35, 'bundle', 108),
  ('BUNDLE-009', 'Georges House Pride Bundle', 20, 35, 'bundle', 109),
  ('BUNDLE-010', 'Patricks House Pride Bundle', 20, 35, 'bundle', 110),
  ('BUNDLE-011', 'Davids Premium House Bundle', 20, 45, 'bundle', 111),
  ('BUNDLE-012', 'Georges Premium House Bundle', 20, 45, 'bundle', 112),
  ('BUNDLE-013', 'Patricks Premium House Bundle', 20, 45, 'bundle', 113),
  ('BUNDLE-014', 'Andrews Premium House Bundle', 20, 45, 'bundle', 114)
...
where component_sku in ('MERCH-005', 'MERCH-006', 'MERCH-007', 'MERCH-008');

delete from merch_products product
where product.sku in ('MERCH-005', 'MERCH-006', 'MERCH-007', 'MERCH-008')
  and not exists (
    select 1
    from merch_reservation_items item
    where item.sku = product.sku
  );

update merch_products product
set total_quantity = 0,
    product_type = 'retired',
    updated_at = now()
where product.sku in ('MERCH-005', 'MERCH-006', 'MERCH-007', 'MERCH-008');
...
begin
  ...
  insert into merch_reservation_items (reservation_id, sku, product_name, size, color, quantity, unit_price_usd)
  select
  ...
  return jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'items', (
        select coalesce(jsonb_agg(jsonb_build_object(
          ...
          'lineTotal', item.unit_price_usd * item.quantity
    ...
    jsonb_build_object(
      'currency', 'USD',
      'subtotal', (
        select coalesce(sum(item.unit_price_usd * item.quantity), 0)
        from merch_reservation_items item
        where item.reservation_id = created_reservation.id
      ),
      'total', (
...
alter table merch_bundle_components enable row level security;
alter table merch_reservations enable row level security;
alter table merch_reservation_items enable row level security;

revoke all on function reserve_merchandise_order(jsonb) from public;
revoke all on merchandise_inventory_summary from public;

do $$
begin
  if exists (select 1 from pg_roles where rolname = 'anon') then
    revoke all on merch_products from anon;
    revoke all on merch_bundle_components from anon;
    revoke all on merch_reservations from anon;
    revoke all on merch_reservation_items from anon;
    revoke all on merchandise_inventory_summary from anon;
    revoke execute on function reserve_merchandise_order(jsonb) from anon;
  end if;

  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    revoke all on merch_products from authenticated;
    revoke all on merch_bundle_components from authenticated;
    revoke all on merch_reservations from authenticated;
    revoke all on merch_reservation_items from authenticated;
    revoke all on merchandise_inventory_summary from authenticated;
    revoke execute on function reserve_merchandise_order(jsonb) from authenticated;
  end if;

  if exists (select 1 from pg_roles where rolname = 'service_role') then
    grant all on merch_products to service_role;
    grant all on merch_bundle_components to service_role;
    grant all on merch_reservations to service_role;
    grant all on merch_reservation_items to service_role;
    grant select on merchandise_inventory_summary to service_role;
    grant execute on function reserve_merchandise_order(jsonb) to service_role;
  end if;
end;
$$;
